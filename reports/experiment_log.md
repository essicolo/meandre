...
## 2026-08-21 — LES CONTRAINTES AUXILIAIRES NE CONTRAIGNAIENT RIEN : audit, correctifs, cible CanSWE

Journée partie d'une remarque d'Essi qui n'admettait pas de réponse commode : « tu as tout testé il me semble. il manque d'eau, ce n'est ni le modèle ni la météo, ce qui n'a aucun sens ». Elle avait raison, et la réponse est que le modèle ne manque pas d'eau du tout.

DEUX HYPOTHÈSES STRUCTURELLES ÉLIMINÉES, sans aucune simulation. R18 : le domaine modélisé est bon, l'aire cumulée au nœud de la jauge vaut 1.005 fois l'aire officielle sur 160 stations et six régions, sans biais régional. Mon premier chiffre annonçait 0.248 et un désastre ; il lisait `drainage_area_km2` du parquet territorial, qui n'est ni l'aire locale ni la cumulée (14 926 km² sommés pour un domaine de 83 202). Cinquième colonne mal nommée de la série, consignée en dette #13. R20 : le printemps n'est jamais limité par l'apport, l'écoulement d'avril-mai vaut 0.31 à 0.45 de la neige plus la pluie, et la neige n'y est que MINORITAIRE (0.29 à 0.52) — viser la masse du manteau, c'était viser la minorité de l'apport.

R19, ÉTABLI SANS MODÈLE : de décembre à mars, le débit observé n'est pas mesuré. Part des jours situés sur un segment parfaitement rectiligne (dérivée seconde nulle) : février 21.7 %, janvier 14.7, mars 12.8, décembre 6.6, contre 0.3 à 0.6 % de mai à novembre. Signature d'une interpolation tracée à la main entre jaugeages sous couvert de glace, et c'est un PLANCHER. Nous ajustons donc une partie de l'hiver sur une reconstruction du CEHQ, tout comme Hydrotel, calé sur les mêmes séries. N'explique pas avril, qui est un mois mesuré.

R21 puis R22, LE CADRAGE DU CHANTIER. Le cycle saisonnier des stocks (nouveau mode `ETL_STOCKS=1`) montre un modèle qui n'a QU'UN réservoir saisonnier, le manteau : nappe 0 mm toute l'année, sol L3 à 1044 mm avec 14 mm d'amplitude et +1 mm de décembre à avril (plein à 91 % de sa porosité, masse morte). En décembre, 47.7 mm atteignent le sol et 45.0 ressortent : rien n'est retenu. Mais GRACE corrige la conclusion dans l'heure : l'amplitude totale du modèle (146 mm) vaut 102 % de la mesure (143 mm). Le modèle stocke la BONNE QUANTITÉ par le MAUVAIS CHEMIN, et sa phase est décalée d'un mois (mai : -6 mm chez nous, +45 pour GRACE, le plus gros écart de l'année).

R23, POURQUOI GRACE NE DISAIT RIEN. `w_tws = 0.2` était actif depuis toujours dans la config de base. `tws_anomaly_loss` compare des mois INDIVIDUELS à sigma=25 mm, l'incertitude d'UNE estimation. Le résidu climatologique vaut 26.2 mm, soit 1.05 écart-type : la perte se déclarait satisfaite. Avec ~21 ans par mois calendaire, l'incertitude de la climatologie est 5.4 mm et le même résidu vaut 4.8 écarts-types (mai 9.2, juin 7.5). Une erreur saisonnière systématique moyennée sur vingt ans ne se juge pas à l'incertitude d'un mois.

R24, L'AUDIT DES TROIS (nouveau mode `ETL_AUX=1`, même moule pour chacune : part systématique du résidu, significativité contre la dispersion interannuelle). MODIS ET : 77 % systématique, jusqu'à 27 écarts-types, -117 mm/an. MODIS couverture nivale : 89 % systématique, 38 écarts-types en mars, et le modèle voit PLUS de neige que le satellite. GRACE : 65 % systématique, 8.1 écarts-types en juin. Conclusion qui n'était pas attendue : réparer naïvement la contrainte neige aurait EMPIRÉ le modèle, puisqu'elle lui demandait de fondre plus tôt, contre GRACE. Le bug était accidentellement protecteur. CanSWE tranche : pic mesuré 238 mm quand le modèle en a 121. Donc la réalité met 238 mm de neige compensés par un sol qui se vide d'environ 95 mm ; nous mettons 121 mm et un sol plat. Même total, physique opposée.

QUATRE DÉFAUTS SILENCIEUX, tous de la famille « du code qui affirme sans lire ». Dette #14 : `with_forcing` perdait `swe_obs` depuis le commit du 21 juillet, celui-là même qui ajoutait la supervision MOD10 ; la perte neige ne s'est jamais évaluée en 219 commits, pendant que le pilote imprimait `w_snow = 0.3`. Dette #15 (trouvée par une question d'Essi, « quel module ET est entraîné sur Modis 16 ? le MLP a été écarté il me semble ») : le journal imprimait `etp_channel=6` EN DUR, y compris sous Linacre qui pose `None` — j'avais moi-même bâti un raisonnement faux dessus. Plus la perte GRACE qui lisait une épaisseur de couche codée en dur (0.30 au lieu de 0.15) et le champ de milieu humide MORT au lieu de `wet_vol`.

CORRECTIFS, 212 tests verts. `with_forcing` passe par `dataclasses.replace`, qui ne peut rien oublier. Le `Trainer` LÈVE si un poids auxiliaire est positif sans sa cible. Terme climatologique GRACE `w_tws_clim` (passage direct : valeur = biais accumulé par mois calendaire, gradient = résidu courant, donc calculable tronçon par tronçon). ET en mode ANOMALIE — MOD16 pour la forme, jamais pour le niveau, puisque le modèle évapore 422 mm/an dans la fourchette boréale 400-500 des tours à flux quand MOD16 impliquerait 539. Nouvelle cible `w_swe_mass` sur la MASSE du manteau (CanSWE, `build_swe_targets`, filtres 15 km et 150 m ; 97 sites retenus sur OUTV, 37 sagu, 33 slno, 49 gasp, 12 abit, aucun sur mont et slso où le poids s'éteint de lui-même).

À SUIVRE : file `file_auxiliaires.sh`, runs A (GRACE clim + correctifs + ET anomalie) et B (A + masse CanSWE). Le critère n'est pas le score — Essi : « pas tant pour le kge que l'identifiabilité ». Cibles : mai -6 -> +45 mm, pic de manteau 121 -> 238 mm, avril 0.753 -> 1.0. Contrôle du champion à 0.8088 dans ce protocole (0.7880 au registre, l'écart tient à `ETL_KGW`).

## 2026-08-22 (nuit) — TROIS CORRECTIONS DE MES PROPRES CONCLUSIONS, dont une en cascade

Suite de la journée du 21. Les runs aux-A et aux-B tournent ; le travail de nuit a surtout consisté à défaire ce que j'avais affirmé trop vite.

DRAPEAUX CEHQ INGÉRÉS (tâche 49 close). `meandre/data/cehq_loader.py` + `.runs/quebec/ingest_cehq_flags.py`, colonnes `remark` et `reconstructed` dans `observations`, 13 tests. 13 régions, 173 stations, 1.16 M valeurs. R19 passe de la déduction à la MESURE : en tenue de côté sur OUTV, janvier est reconstruit à 85.4 %, février 87.3, mars 60.8, décembre 47.0, avril 9.5, zéro de mai à octobre. Ma détection indirecte par segments rectilignes donnait 14-22 % en février ; je l'avais annoncée comme un plancher, le vrai chiffre est 87. Conséquence qui tranche deux chantiers : le déficit d'avril porte sur des jours MESURÉS et reste entier, l'écart d'hiver se compare surtout à une reconstruction.

R26 : LA COMPARAISON AVEC HYDROTEL SURVIT. Crainte immédiate après R19 : Hydrotel est calé sur ces mêmes reconstructions, donc son avance hivernale serait un artefact. Testé sans faire tourner méandre (`.runs/quebec/hydrotel_vs_flags.py`). Premier test écarté parce que mal construit — jours mesurés 0.799 contre reconstruits 0.594, mais les jours reconstruits SONT les jours d'hiver, donc ça oppose deux saisons. Test valide, intramensuel : écart médian -0.061 sur 14 couples région x mois, cinq positifs sur quatorze. Le drapeau ne prédit rien. Aucun ajustement à la reconstruction, et pas davantage de preuve que ces jours seraient plus difficiles (une première passe à 10 couples donnait -0.135 et je l'avais lu ainsi ; à 14 couples c'est sous la dispersion).

R25 : LE SOL, PAS LA PHÉNOLOGIE. Essi avait relevé que l'identifiabilité de l'ET passe aussi par la météo et le sol. Nouveau diagnostic ETR/ETP et saturation par couche : le rapport est PLAT toute l'année entre 0.81 et 0.88, le modèle n'est jamais limité par l'eau, et la couche L3 ne respire pas. Le résidu d'ET a donc deux causes distinctes : juin-juillet côté demande (le rapport plafonne), septembre-octobre côté offre (le modèle puise dans une couche qui ne s'assèche jamais). Blâmer la phénologie était faux.

R28 : LE 238 mm ÉTAIT UN ARTEFACT, ET TROIS CONCLUSIONS TOMBENT AVEC. J'avais pris comme pic de manteau mesuré le maximum sur l'hiver de la MÉDIANE JOURNALIÈRE du réseau CanSWE. Or les sites qui rapportent changent d'un jour à l'autre, et le rapport pic/neige-tombée corrèle à +0.58 avec l'altitude du site : cette statistique retenait le jour où les sites les plus enneigés rapportaient. Statistique correcte (pic par site et par saison, puis médiane sur les sites) : 108 mm sur OUTV, sur 71 sites. Le modèle en accumule 121, donc il a LÉGÈREMENT PLUS de neige que la mesure. Vérification croisée site par site : rapport médian pic mesuré / neige tombée au nœud = 0.75, avec 53 sites sur 69 sous 1.0, ce qui est physiquement normal. Aucun sous-captage sur OUTV. Ce qui survit ailleurs : sagu 1.21 et slno 1.25, pics mesurés supérieurs à la neige disponible, vrai problème là-bas.

Cascade : le volet neige de R24 est void ; la prémisse de R27 est voidée (même si sa décision opérationnelle, ne pas imposer CanSWE en niveau, reste bonne pour sagu et slno) ; et R21 perd son argument de COMPOSITION, puisqu'avec un manteau réel de ~108 mm pour 143 mm d'amplitude GRACE, la réalité met ~35 mm dans le sol et la nappe quand le modèle en met 25. Ce qui reste solide du chantier stockage : la PHASE (R22), pic en mars contre avril, 51 mm d'écart en mai. Leçon : j'ai utilisé ce 238 dans trois raisonnements successifs sans jamais vérifier comment il était calculé.

RUN B RECONFIGURÉ AVANT DÉMARRAGE. Il devait imposer la masse CanSWE comme cible de niveau ; il fera l'ablation du terme climatologique GRACE, seule façon de savoir ce que ce terme apporte dans A. La cible CanSWE reste câblée et testée, en attente d'un mode ANOMALIE (la forme, pas le niveau).

DETTE #16 : `patience` n'a JAMAIS atteint le pilote québécois. `etl_run.py` construit `TrainingConfig` en énumérant les clés du TOML et l'oublie ; elle reste à 0, donc l'arrêt anticipé ne s'est jamais déclenché, et les 53 configs portant `patience = 8` depuis le 21 n'y changent rien. SLSO est indemne. Non corrigé pendant la file, pour que B tourne sous le même protocole que A. Troisième instance du même motif en 24 h (avec #14 `with_forcing` et #15 l'affichage `etp_channel`) : une CONSTRUCTION PAR ÉNUMÉRATION perd en silence tout ce qu'on ajoute ailleurs.

Index de mémoire compacté de 31.9 à 19.3 ko : il dépassait sa limite et n'était chargé qu'en partie, donc des entrées disparaissaient au hasard à chaque session.

## 2026-08-22 — OUVERTURE ASSUMÉE AU-DELÀ DE LA FIDÉLITÉ : fonte saisonnière + sublimation, opt-in, jugées sur CanSWE

Décision d'Essi : « maintenant qu'on se dégage de l'appariement avec Hydrotel, on pourrait très bien ajouter des processus qui permettraient de capter les irrégularités », avec pistes explicites (sublimation, modulation du coefficient de fonte, drainage souterrain) et renvoi à Raven et à la littérature. C'est la deuxième phase prévue par la discipline du dépôt : la baseline fidèle est validée, la modernisation se fait pièce par pièce, chaque ajout ciblé sur un écart MESURÉ avec son observable propre.

DEUX PROCESSUS LIVRÉS, opt-in, fidélité par défaut verrouillée par 12 tests (254 au total).

FONTE SAISONNIÈRE (hydrotel_clone/snow.py, `melt_seasonal_amp`). Un facteur degré-jour constant absorbe implicitement le cycle annuel de radiation ; calé sur la crue, il fond trop en novembre-décembre. C'est le déficit mesuré (R32) : accumulation à 58 % de CanSWE, fonte de cœur d'hiver presque juste (27.1 % contre 22.9 mesuré, 24.6 de référence), date de disparition juste. Correctif classique d'Anderson SNOW-17, repris par POTMELT_HBV de Raven : s(j) = 1 + amp·sin(2π(j-81)/365), minimum au solstice d'hiver, moyenne annuelle 1 donc le taux calé reste le taux moyen. ETL_MELT_SAISON=amp.

SUBLIMATION (Kuzmin 1957, SUBLIM_KUZMIN de Raven, `sublimation_mode="kuzmin"`). E = (0.18+0.098·u)·(e_s glace − e_a), ne demande que u2 et e_a déjà dans le forçage ; saturation sur GLACE (Magnus-Tetens 21.87/265.5, l'écart eau/glace atteint 20 % à −20°). 15-40 mm/hiver attendus en forêt boréale. SORTIE ATMOSPHÉRIQUE exposée de bout en bout (diag subl_mm → SimDiagnostics.sublimation → terme du bilan ETL_BILAN) : sans ça l'audit lirait une fuite, le piège exact du milieu humide (dette #12). Un test verrouille la conservation : stock_sans − stock_avec == cumul déclaré. ETL_SUBLIM=1.

RÉPARÉ AU PASSAGE : le smoke standalone de la colonne était cassé depuis MON refactoring phénologie du 19 août (cache sur ref.shape[0] avec un krec 0-d) — il n'est pas collecté par pytest, personne ne l'a vu. Dette #16 close le matin même : patience passait enfin au pilote, et le balayage a attrapé un second bug du même motif (grad_clip lu sous le nom clip_grad_norm, bénin). Test de famille écrit : toute clé [training] ignorée en silence fait échouer la suite.

FILE GPU (~11 h) : aux-B (ablation du terme GRACE climatologique — déjà instructif : B subit la MÊME excursion violente que A, régression 23.4 %, donc l'excursion ne vient PAS de w_tws_clim) → res-C1/C2 (résidence : krec appris ancré, k_gw 0.0273 puis 0.0090, GRACE juge) → seuil-D1/D2 (+0.3 dérivé de CanSWE contre −2.2168 contrôle) → proc-E1..E4+E0 (fonte saisonnière 0.5/0.3, sublimation, les trois leviers nivaux ensemble, contrôle). Tout se juge d'abord sur « neige, simule/mesure par mois » (cible 11=0.66, 12=0.55 vers 1.0) et sur la phase GRACE, le KGE en dernier.

NOTE : aux-A a tourné DEUX FOIS par ma faute (file éditée pendant exécution — bash relit au décalage d'octets). Les deux passages rendent exactement 0.7932/0.7723 : reproductibilité bout en bout prouvée, à défaut d'autre chose. aux-A lui-même : NEUTRE sur le score (0.7932 contre 0.7880, sous le bruit), NÉGATIF sur le volume (−8.1 % annuel, l'ET anomalie sur-évapore l'été), R31.

## 2026-08-22 (après-midi) — TROIS PAIRES LUES : l'ablation GRACE, la résidence, et le seuil qui gagne sur tous les tableaux

R33, ABLATION aux-A/aux-B (appariée, même graine). Le terme climatologique GRACE ne coûte rien (+0.0105 apparié) mais ne répare RIEN de la phase : biais de mai −45.0 contre −45.4, identiques. Raison structurelle : krec est imposé par le calage, hors du graphe de gradient — aucun poids de perte ne remplit un réservoir dont la vanne est fermée. Complément exact de R11 : le débit ne VEUT pas de réserve, GRACE la veut mais ne PEUT pas l'obtenir sans le degré de liberté. Au passage, premier chiffre des jours mesurés (tâche 48 close) : sur les jours non reconstruits, méandre 0.7928 est à PARITÉ avec la médiane de l'ensemble Hydrotel (0.7990) et bat deux membres sur six.

R34, RÉSIDENCE res-C1/res-C2 (krec APPRIS par le champ, moyenne ancrée 2e-5 — demande d'Essi « krec devrait être dans le nerf » ; k_gw 0.0273 puis 0.0090 ; GRACE en juge hors de la perte). PREMIÈRE NAPPE NON NULLE DU PROJET : 7-9 mm (C1), 28-34 mm (C2, l'échelle suit S=Q/k). L3 respire enfin (amplitude 37 mm contre 14). La phase GRACE bouge pour la première fois : résidu systématique 26.3 → 22.2 mm (−16 %), par la physique seule. Coût : −0.01 à −0.03 de KGE, l'arbitrage R11 chiffré. MAIS la nappe est PLATE (déc 30 → avr 29) : la recharge q3 = krec·z3·θ3 est quasi constante puisque θ3 ne varie qu'entre 0.96 et 1.00 — un réservoir linéaire alimenté en continu ne porte aucune saisonnalité, quel que soit k_gw. Le levier suivant est la RESPIRATION de L3, pas la vidange de la nappe. Le réservoir puissance (Wittenberg, BASE_POWER_LAW) est écrit et testé (7 tests), en attente.

R35, SEUIL seuil-D1/seuil-D2 — LE RÉSULTAT LE PLUS PROPRE DE LA CAMPAGNE. La valeur +0.3 °C, DÉRIVÉE de CanSWE avant tout run et dans la plage littérature, contre le contrôle apparié à −2.2168 (héritage du calage Hydrotel) : déficit d'accumulation EFFACÉ (novembre 0.66 → 1.09, décembre 0.55 → 0.96, le contrôle reste à 0.52-0.67), accumulation appariée 77 % contre 58, pertes hivernales justes (25.4 contre 24.6 de référence), débit de décembre 1.235 → 0.999, avril 0.753 → 0.819, gamma 0.9245 → 1.0014, et held-out 0.8014 contre 0.7905 au contrôle et 0.7885 au champion. La physique dérivée d'une mesure bat la compensation héritée MÊME au score. Reste : mai déborde à 1.292 (la neige gagnée fond tard) — c'est le travail de la fonte saisonnière, série E en cours.

Note transversale : l'excursion violente + redémarrage autopilote a frappé les QUATRE entraînements (A, B, C1, C2), avec et sans chacun des termes nouveaux. Facteur commun : la montée du taux d'apprentissage vers 5e-4. Le garde-fou la rattrape à chaque fois ; à calmer un jour par un plafond de LR, pas urgent.

## 2026-08-22 (soir) — LE SEUIL GÉNÉRALISÉ AU BULBE HUMIDE, ET L'INTÉGRATION QUI RÉPARE LE NIVAL

Question d'Essi sur R35 : le +0.3 est régional, comment généraliser ? Réponse implémentée et validée le jour même. La variable physique du partage pluie-neige est le BULBE HUMIDE (Jennings 2018 : les seuils AIR varient de 0.6 à 3.8 °C dans l'hémisphère nord et la variance est l'humidité relative). Vérifié sur nos 6 régions : le seuil Twb équivalent au +0.3 air d'OUTV tient dans [-1.0, -0.7] partout. Implémenté opt-in (split_mode wet_bulb, Stull 2011, e_a du forçage, 3 tests), levier ETL_SEUIL_TWB. F1 (inférence pure) : Twb −0.8 reproduit les gains du seuil air sur OUTV. Un seul nombre, aucun paramètre régional.

SÉRIE E (R36) : E0 contrôle exact, banc valide. Fonte saisonnière amp 0.5 retenue (mai dégonflé 1.14 → 0.88, début d'hiver remonté). Sublimation Kuzmin brute REJETÉE : elle mange le manteau (décembre 0.40) — vent de plaine sous 74 % de forêt ; à reprendre un jour avec un facteur d'abri par canopée.

INT1 (R37), tous les gagnants ensemble — Twb −0.8, amp 0.5, krec appris ancré, k_gw 0.0273, w_tws_clim actif (krec enfin dans le graphe, le test que R33 réclamait), w_et 0.4 — arrêté par patience à ~11 époques (premier arrêt anticipé du pilote québécois). LE NIVAL EST RÉPARÉ : CanSWE 1.10/0.99/1.00/0.93/0.95 de novembre à mars (0.66/0.55/0.65/0.66/0.70 au champion), pertes 19.7 % contre 22.9 mesuré, disparition +1/0, quartiles forestiers plats à 1.0. Débit : avril 0.753 → 0.865, décembre 1.235 → 0.897, gamma 1.0416. Score : 0.7963 tous jours, 0.8090 sur les jours mesurés — au-dessus du champion avec une physique défendable.

CE QUI RESTE POUR LA SUITE DES 48 H : mai déborde encore (1.235) ; la phase GRACE s'est DÉGRADÉE (mars +58, résidu 30.2) — CanSWE dit la neige de mars juste, GRACE dit le stockage TOTAL trop haut, donc la compensation réelle est un SOL qui se vide en hiver, et L3 ne respire toujours pas (nappe 9-12 mm plate, même avec le terme GRACE actif : on ne peut pas apprendre une recharge saisonnière si theta3 est épinglé). Prochain levier structurel : le drainage NON LINÉAIRE au niveau de L3 (PowerLawAquifer écrit et testé, 7 tests) — déplacer la non-linéarité de la nappe vers la vanne de L3, pour que la couche se désature en étiage et que la fonte la re-remplisse au printemps.

## 2026-08-22 (nuit) — LA FRONTIÈRE SCORE-PHASE EST MESURÉE, ET ELLE DIT OÙ NE PAS CHERCHER

Suite du programme de modernisation, bancs G0 à G8 plus deux entraînements (INT2, INT3), tous en une soirée grâce aux runs d'inférence à 6-9 minutes.

R38 : l'exposant non linéaire SEUL sur le drainage de L3 ne fait RIEN (G1-G3 identiques au contrôle, INT2 = INT1) — j'avais préservé le plafond de drainage par principe alors que le plafond EST la contrainte, le flux sortant étant déjà à son maximum toute l'année. Plafond relevé (krec 1e-4) + exposant 8 : L3 respire de 95 mm et le résidu systématique GRACE tombe à 11.2 mm, le plus bas jamais mesuré (30.2 à INT1). La phase est réparable par la physique.

MAIS (R39) : toute absorption de la crue vers le souterrain VOLE avril. INT3 entraîné (krec 7e-5, n=8) : GRACE 19.8 mais avril 0.590 et r effondré à 0.745, score 0.645. La nappe puissance par-dessus (G7/G8) : avril 0.473, restitution culminant en juin-juillet — la chaîne souterraine entière rend l'eau un à deux mois trop tard. Frontière du jour : 0.796/30.2 (INT1), 0.645/19.8 (INT3), 0.36/11.2 (G5).

Le conflit est structurel : les jauges veulent l'eau de fonte EN AVRIL, GRACE veut du stockage tenu en avril-mai. Les deux ensemble exigent une voie que la colonne n'a pas — un stockage RAPIDE-lent de quelques semaines (milieux humides, plaines d'inondation), pas le souterrain profond. Ou alors le biais de mars de GRACE est en partie un artefact d'empreinte (~300 km de pixel contre notre masque). Les deux hypothèses s'instruisent par ANALYSE, pas par GPU : c'est le programme du jour 2.

RECETTE À RETENIR en attendant : INT1 — nival réparé sans paramètre régional (CanSWE ~1.0 tout l'hiver), bulbe humide Twb −0.8, fonte saisonnière 0.5, krec appris ancré 2e-5, 0.7963 tous jours et 0.8090 sur les jours mesurés, au-dessus du champion. Modules livrés et testés aujourd'hui : fonte saisonnière, sublimation (rejetée en l'état, à abriter par la canopée), bulbe humide, drainage L3 non linéaire, nappe puissance — tous opt-in, fidélité par défaut, 91 tests sur les blocs training+vertical.

## 2026-08-23 (matin) — COMMITS, GRACE DISCULPÉE, MILIEU HUMIDE RÉFUTÉ, ET LA FLOTTE DE GÉNÉRALISATION

Sur demande d'Essi : quatre commits thématiques (contraintes auxiliaires + drapeaux CEHQ ; les 5 processus opt-in ; pilote + registre ; chantier forçage SLSO du 18 en attente depuis sa revue), plus le retrait des 8 constructeurs réfutés déjà commité la veille.

R40, deux verdicts d'analyse sans GPU. GRACE AU MASQUE : le bassin ne remplit que 50-59 % de sa bbox, mais la série pondérée par le masque (mascons CSR 0.25° en cache) corrèle à 0.984-0.999 avec la bbox et ne déplace le cycle d'OUTV que de 2 mm — le mars à +58 est un vrai signal, pas un artefact d'agrégation ; seule GASP bouge (+11.8 mm en avril). MILIEU HUMIDE : allonger c_prod à 30 ou 60 jours ne change RIEN (stock 2-8 mm identique) — le réservoir ne dépasse jamais son volume normal, seul domaine où c_prod agit. Troisième voie de rétention réfutée après le sol profond et la nappe. Le décalage résiduel avril/mai (~2 semaines de fonte-vers-débit) reste ouvert ; candidats non traités : forme intra-avril de la fonte, plaine d'inondation au routage.

PRIORITÉ DÉPLACÉE : flotte de GÉNÉRALISATION lancée — gasp, sagu, slno avec la recette INT1 telle quelle. Aucun bouton régional : Twb −0.8 unique, fonte saisonnière universelle, krec appris ancré littérature, k_gw par le champ provincial (GP 127 stations) au lieu du scalaire d'OUTV. Les seuls ancrages sont les paquets de plateforme (loi des ancrages). Juges : la ligne CanSWE de chaque région d'abord — si le nival se répare AILLEURS sans rien retoucher, la thèse du projet (la physique + le champ portent la géographie) tient son premier test grandeur nature.

## 2026-08-23 (après-midi) — LA RECETTE GÉNÉRALISE : cinq régions sans un seul bouton régional

FLOTTE gen1 (Twb −0.8 unique, fonte saisonnière 0.5, krec appris ancré littérature, k_gw champ provincial GP, ET anomalie 0.4, GRACE clim ; seuls ancrages = paquets de plateforme). Résultats held-out contre la meilleure référence méandre de chaque région : GASP 0.7134 contre 0.577 (+0.14) ; MONT 0.6953 contre 0.6243 (+0.07 ; +0.20 contre le diagnostic du 21) — la pire région du projet rejoint la zone Hydrotel ; SLNO 0.7631 contre 0.7106 (+0.05) ; SAGU 0.7438 contre 0.7587 (parité) ; ABIT 0.5244 contre 0.5475 (parité, n=3 et gamma 0.586 : chantier lacs, tâche 40). R41.

NIVAL : SAGU (0.69-0.79) et SLNO (0.80-0.98) sont à leur PLAFOND DE FORÇAGE (R28 : le pic CanSWE y dépasse la neige que CaSR fait tomber, rapports 1.21-1.25 — sous-captage). L'anomalie est GASP, manteau à 1.8-2.5x la mesure. R42 : trois suspects éliminés par mesure (seuil — hors de portée d'un facteur 2 ; fonte saisonnière — ablation nulle et l'amplitude AIDE le score ; appariement — écarts d'altitude −36 m médians). Restent les taux de fonte ancrés face à la fonte de mi-hiver maritime, et la phase CaSR côtière. Sortie propre pressentie : le mode ETI (radiation réelle), qui dissoudrait l'amplitude sinusoïdale ET ce problème dans la même variable — même mouvement que le bulbe humide pour le seuil. Note : CanSWE gaspésien n'a AUCUN relevé de novembre-décembre.

FLOTTE gen3 lancée sur les 7 régions restantes (cnda-e, labi, vaud). GRACE au masque, milieu humide réfuté et commits du matin déjà consignés plus haut (R40).

## 2026-08-24 (nuit) — CARTE PROVINCIALE COMPLÈTE, ET LA PHYSIQUE DE L'ETI POSÉE PROPREMENT

FLOTTE gen3, sept régions restantes, même recette unique : cnda 0.7849, cndb 0.6993, cndc 0.7222, cndd 0.8057, cnde 0.5281, labi 0.5035 (contre 0.5567 avant, n=1), vaud n=0 (aucune station à 365 jours valides). Ce sont les PREMIÈRES références de ces régions par ce pilote. Douze régions courues avec une seule recette — R41 complété.
...
## 2026-09-20 — UN PROTOCOLE RÉPARÉ DEUX FOIS, ET UN SOL QUI SE DÉCLARE

PROTOCOLE, PREMIÈRE RÉPARATION : le taux d'apprentissage par défaut du pilote régional vaut 5e-4, et trente-quatre scripts sur trente-six le laissaient en place. À ce taux, un affinage de huit époques est BISTABLE : six passes identiques ont donné exactement deux valeurs de KGE tenu de côté, 0,7056 et 0,5975, sans rapport avec la graine, et la meilleure validation donnait le pire tenu de côté. La dispersion tombe à 0,0075 à 3e-5 et à 0,0000 à 1e-5. Les passes avaient d'abord été décrites comme tournant à 1e-4, par erreur d'étiquette de ma part ; le barème monte du millième du taux jusqu'au taux et ne multiplie rien. Vingt-trois rondes sur trente et une étaient des passes avant à poids gelés et tiennent ; huit entraînaient au taux bistable et leurs verdicts sont retirés. R144.

PROTOCOLE, SECONDE RÉPARATION, et c'est la trouvaille du jour. `MEANDRE_DIAG_CPU=1` déplaçait les diagnostics sur le processeur en les DÉTACHANT du graphe, au motif écrit dans le code qu'aucune perte n'en dérive. C'était vrai à son écriture, deux jours plus tôt. Depuis, quatre termes en dérivent : l'évapotranspiration MODIS de `etr`, la neige CanSWE de `swe`, les niveaux de puits de `s_gw`, la gravimétrie GRACE de `theta`, `swe` et `s_gw`. Ils gardaient leur valeur, s'imprimaient dans le journal, et ne produisaient aucun gradient. Symptôme qui a conduit là : deux passes du Saint-Laurent nord-ouest, l'une contrainte et l'autre non, identiques à quatre décimales à CHAQUE époque.

EFFET DU CORRECTIF, mesuré à configuration identique : les contraintes auxiliaires coûtent 0,032 de KGE et DIVISENT PAR QUATRE la dispersion entre tirages, de 0,0141 à 0,0036. L'identifiabilité se mesure donc ici sur la reproductibilité, pas sur la performance. R148.

CARRÉ PHYSIQUE CONTRE CONTRAINTE, huit passes, deux graines par case : aucun effet sur le débit ne sort du bruit, la plus grande dispersion de case valant 0,0683. Le résultat est ailleurs et il n'était pas cherché : les cases portant la physique corrigée se répliquent à quatre et sept millièmes, les cases témoin à vingt-deux et soixante-huit. J'ai d'abord attribué cet écart aux blocs jetés pour gradient non fini ; le témoin à 1e-5 n'en jette aucun et disperse encore de 0,0141. La correspondance annoncée comme exacte ne l'est pas. R146.

RÉCESSIONS OBSERVÉES, 94 stations, sans aucune simulation : l'exposant de Brutsaert-Nieber vaut 1,65 en médiane, donc un réservoir linéaire unique est exclu, et les temps de vidange s'étalent d'un facteur 5,4. Mais la non-linéarité explique à elle seule 72 % de cet étalement en logarithme : empiler des compartiments pour fabriquer un spectre est inutile. Et 1,65 est proche du 1,5 de Boussinesq en temps long, la loi déjà implémentée. R147.

COVARIABLES : la texture du sol prédit la PART souterraine du débit à +27 % contre le témoin, la géologie du socle à +3 %, le relief à +11 %. Aucune ne prédit la DYNAMIQUE, l'exposant sortant à −8 % et les temps de vidange à +3 % et −19 %. Le chantier de spatialisation se coupe donc en deux. Au passage, la mesure de la veille par la part de variance expliquée était dominée par deux territoires d'une et deux stations et concluait à tort que le socle prédisait l'indice. R145, R149.

FONCTION DE PERTE : huit termes de débit, mais trois directions indépendantes portent 90 % de leur variation. Le KGE est expliqué à 96,9 % par les autres, l'écart quadratique à 99,1 %, et écart quadratique et pics corrèlent à 0,99. Le plus grand angle mort de la recette est le soutien d'étiage, 48,2 %, dont le poids est nul. À nombre de termes égal, décomposer le KGE fait passer le manque moyen de 14,2 % à 2,8 %.

LIVRÉ : processus de sol déclarés par couche avec un catalogue portant les noms de Raven, la déclaration d'Hydrotel reproduisant le clone au bit près sous test ; le plafond de percolation du substratum devenu la quarante-troisième sortie du champ spatial, adressable depuis un profil par son nom ; les trois facteurs du KGE portés séparément dans la perte ; la table des poids de l'affichage corrigée, qui ne couvrait que huit termes sur vingt et imprimait le KGE, l'écart quadratique et le biais BRUTS sous un en-tête annonçant des valeurs pondérées ; cinq bancs de diagnostic nouveaux. Trois cent quatre-vingts tests.
