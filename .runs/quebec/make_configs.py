...
Path(f"{_DATA_ROOT}/quebec/results").mkdir(exist_ok=True)
...
for reg in REGIONS:
    r = reg.lower()
    s = BASE
    s = re.sub(r'basin_db = "[^"]*"', f'basin_db = f"{_DATA_ROOT}/quebec/{r}.duckdb"', s)
    s = re.sub(r'basin_db_pc = "[^"]*"', f'basin_db_pc = f"{_DATA_ROOT}/quebec/{r}.duckdb"', s)
    s = re.sub(r'forcing_cache = "[^"]*"', f'forcing_cache = f"{_DATA_ROOT}/quebec/forcing-{r}.nc"', s)
    s = re.sub(r'checkpoint = "[^"]*"', f'checkpoint = "checkpoints/best-{r}.pt"', s)
    s = re.sub(r'fields_nc = "[^"]*"', f'fields_nc = f"{_DATA_ROOT}/quebec/results/fields-{r}.nc"', s)
    s = re.sub(r'reach_parquet = "[^"]*"', f'reach_parquet = f"{_DATA_ROOT}/quebec/results/reach-{r}.parquet"', s)
