...
def main():
    f = f"{NEISIM}/NEISIM_QCMERI_EENEIG_GOENS_OP2025_MEDIAN.nc"
    ...
    lignes = []
    for k in range(n):
        # Point de grille le plus proche, puis moyenne journaliere des pas de trois heures.
        serie = d.een.sel(lon=lon_s[k], lat=lat_s[k], method="nearest")
        s = pd.Series(serie.values, index=t_n).resample("1D").mean()
        o = pd.Series(snw[k], index=temps_obs).dropna()
