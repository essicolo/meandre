...
run() { local tag="$1"; shift; attendre
  ...
  echo "[pousser] $(date +%H:%M) fini $tag : $(grep -a 'HELD-OUT' "/d/meandre-data/journaux/quebec/log-outv-${tag}.txt" | tr '\n' ' ')"
}
...
PYTHONIOENCODING=utf-8 JOINT_FX_SUFFIX=-hyb MEANDRE_NSUBSTEP=64 \
  ...
  > /d/meandre-data/journaux/quebec/log-diag-ecart-outv.txt 2>&1
