...
# %% [markdown]
# ## 5. Observations hydrométriques (HYDAT — données ouvertes ECCC)

# %%
from meandre.data.open_data import download_observations_hydat, populate_basin_observations

HYDAT_CACHE = GEO_CACHE / "hydat"

hydat_result = download_observations_hydat(
    bbox        = BBOX,
    cache_dir   = HYDAT_CACHE,
    start_date  = cfg["temporal"]["date_start"],
    end_date    = cfg["temporal"]["date_end"],
)

if hydat_result is not None:
    sta_path, obs_path = hydat_result
    n_obs = populate_basin_observations(PC_DB, sta_path, obs_path)
    print(f"HYDAT : {n_obs:,} observations insérées")
else:
    print("[!] HYDAT indisponible — fallback vers stations_concatenees.nc")
    ...
    if STATIONS_FILE.exists():
        ...
        print("[!] Aucune source d'observations disponible")
