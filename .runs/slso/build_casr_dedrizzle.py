"""Dé-crachinage CaSR-référencé : total journalier reconstitué depuis l'horaire (heures>seuil). Quickflow d'intensité RÉELLE depuis l'HORAIRE de CaSR (test sous-journalier scalable).
...
THRESH = float(os.environ.get("DRIZZLE_H", "0.5"))    # mm/h — seuil crachin horaire
...
print(f"noeuds : {len(nc)} | seuil crachin {THRESH} mm/h")
...
for ch in CHUNKS:
    ...
    kept = df.where(df >= THRESH, 0.0)                                 # heures > seuil crachin
    ddaily = kept.resample("1D").sum()                                 # total journalier DÉ-CRACHINÉ mm/j
    daily.append(ddaily)
    print(f"  chunk {ch} : {len(ddaily)} jours, P moy {float(ddaily.values.mean()):.2f} mm/j")
...
out = out[sl]
np.save(".runs/slso/data/casr_dedrizzle_daily.npy", out.values.astype(np.float32))

print(f"[ok] fraction quickflow : {out.shape} | jours pluvieux : {(out.values>0.01).mean()*100:.0f}% "
      f"| P moy/an {out.values.mean()*365.25:.3f}")
