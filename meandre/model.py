...
class YHydro(nn.Module):
    ...
    def __init__(
        ...
        # Routing params (Muskingum K and x) — learned
        # K_musk = (softplus(log_K_musk) + 0.5) * 3600.
        # Initialize so K ≈ 18h (64800s): above the Muskingum c2≥0 threshold
        # of dt/(2(1-x)) ≈ 12h, ensuring non-zero gradients from the start.
        # softplus(x) + 0.5 = 18 → softplus(x) = 17.5 → x ≈ 17.5
        self.log_K_musk = nn.Parameter(torch.full((n_nodes,), 17.5))
        self.logit_x_musk = nn.Parameter(torch.zeros(n_nodes)) # x in (0, 0.5)

    @property
    def K_musk(self) -> Tensor:
        # K > 0 guaranteed by softplus.  Offset of 0.5h (1800s) keeps K > 0
        # for numerical safety.
        return (torch.nn.functional.softplus(self.log_K_musk) + 0.5) * 3600.0

    @property
    def x_musk(self) -> Tensor:
        return 0.5 * torch.sigmoid(self.logit_x_musk)
    ...
    def simulate(
        ...
        for t in range(n_timesteps):
            # Truncated BPTT: detach state to limit backward-pass depth
            ...
            # Detach before buffering: the corrector reads history as observations,
            # not as a gradient path. Without detach the buffer extends the BPTT
            # chain by residual_history steps (90+14=104 steps total), causing
            # float32 overflow (Jacobian^104 → inf) at the epoch the corrector
            # is first enabled.
            ...
            if self.use_residual and self.residual_corrector is not None and len(state_buffer) >= 2:
                history = torch.stack(state_buffer, dim=1)  # (N, H, n_vars)
            ...
            # 3b. Inject AR(1) correlated noise into soil and SWE states
            # Noise enters BEFORE the next physics step so it is filtered by
            # the watershed's inertia (Darcy, ET, routing).
            if noise is not None and self.state_noise is not None:
                noise = self.state_noise.step(noise)   # (n_nodes, 4)
                # Perturb theta1/2/3 (indices 0-2) and SWE (index 3); clamp >= 0
                state = HydroState(
            ...
            # 4. Compute lateral heat load for temperature (if enabled)
            H_lateral = None
            T_air_t = None
            R_n_t = None
            if do_temp:
            ...
            # Update T_water in state if computed
            if T_water_t is not None:
                state = HydroState(
                    theta1=state.theta1,
                    theta2=state.theta2,
                    theta3=state.theta3,
            ...
            if return_diagnostics:
                ...
                # q_upstream: sum of upstream Q_out at each node
                q_up = torch.zeros(self.n_nodes, device=forcing.device)
                if graph.n_edges > 0:
                    q_up.scatter_add_(
                        0, graph.edge_index[1],
                        Q_out_prev[graph.edge_index[0]],
                    )
