...
gru_states      state_date, run_id, data BLOB (pickled numpy h_context)
...
class BasinCache:
    ...
    def save_state(
        ...
        try:
            ...
            con.execute(
                "DELETE FROM gru_states WHERE state_date = ? AND run_id = ?",
                [date, run_id],
            )
            if h_context is not None:
                blob = pickle.dumps(h_context.detach().cpu().numpy())
                con.execute(
                    "INSERT INTO gru_states VALUES (?, ?, ?)",
                    [date, run_id, blob],
                )
        finally:
            con.close()
    ...
    def load_state(
        ...
        try:
            ...
            h_context: Tensor | None = None
            row = con.execute(
                "SELECT data FROM gru_states WHERE state_date = ? AND run_id = ?",
                [date, run_id],
            ).fetchone()
            if row is not None:
