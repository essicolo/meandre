...
class StateResidualCorrector(nn.Module):
    ...
    clamp_min, clamp_max : float
        Physical bounds applied to ``state_history`` before the GRU forward pass.
        Guard against NaN/Inf that can appear from extreme physics states and
        corrupt the GRU hidden state (CUBLAS_STATUS_EXECUTION_FAILED).
        Defaults (-50, 500) cover typical soil moisture and snow water equivalent
        ranges in mm over the simulation window. Override via HydroModel.
    """
    ...
    def __init__(
        ...
        # Linear warm-up multiplier — ramps from 0 -> 1 over the
        # first `residual_warmup_epochs` so the correction is gradually mixed in.
        self.register_buffer(
            "warmup_factor", torch.tensor(1.0), persistent=False
        )
    ...
    def forward(self, state_history: Tensor, physics_state: Tensor) -> Tensor:
        ...
        state_history = torch.clamp(
            state_history, min=self.clamp_min, max=self.clamp_max
        )
        ...
        gate = torch.sigmoid(self.gate_logit) * self.warmup_factor  # (n_state_vars,)
