...
from meandre.data.canswe_loader import _haversine_km, _texte


def test_texte_decode_les_octets():
    ...
    assert list(_texte(a)) == ["", "R", "M"]


def test_texte_tolere_texte_et_none():
    a = np.array(["A", None, " B "], dtype=object)
    assert list(_texte(a)) == ["A", "", "B"]


def test_texte_preserve_la_forme():
    a = np.array([[b"", b"R"], [b"M", b""]], dtype="S1")
    assert _texte(a).shape == (2, 2)
