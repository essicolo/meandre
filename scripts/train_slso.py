...
def main():
    ...
    # Initialize Muskingum K from reach lengths (edge_attr_0) in DuckDB.
    # K ≈ reach_length / velocity.  Assume v ≈ 0.8 m/s (boreal streams).
    import duckdb as _ddb
    _con = _ddb.connect(str(BASIN_DB), read_only=True)
    _edges = _con.execute("SELECT dst, edge_attr_0 FROM edges").fetchall()
    _con.close()
    _reach_len = torch.full((n_nodes,), 500.0, device=device)  # default 500m
    for dst, length in _edges:
        if dst < n_nodes:
            _reach_len[dst] = max(length, 100.0)
    _K_hours = _reach_len / 0.8 / 3600.0  # hours
    _K_hours = torch.clamp(_K_hours, min=1.0, max=48.0)  # reasonable range
    # Invert: K = (softplus(raw) + 0.5) * 3600s → raw such that softplus(raw) = K_hours - 0.5
    # For K_hours > 1.5, softplus(raw) ≈ raw, so raw ≈ K_hours - 0.5
    _raw = _K_hours - 0.5
    with torch.no_grad():
        model.log_K_musk.copy_(_raw)
    print(f"Muskingum K init: mean={_K_hours.mean():.1f}h, min={_K_hours.min():.1f}h, max={_K_hours.max():.1f}h")
    ...
    # Chunk-safe loss: MSE + log-MSE (additive across chunks, no checkpointing).
    # KGE is evaluated at validation time (no backward needed).
    # Precompute per-station variance for normalized MSE (≈ 1-NSE with fixed denom).
    station_var = []
    for si in station_indices:
        col = q_obs_tensor[:train_sl.stop - train_sl.start, station_indices.index(si)]
        valid_vals = col[~torch.isnan(col)]
        station_var.append(valid_vals.var() if len(valid_vals) > 30 else torch.tensor(1.0, device=device))
    ...
    train_cfg = TrainingConfig(
        ...
        chunk_steps=tcfg.get("chunk_steps", 365),
    ...
    # Warm start: load existing checkpoint if architecture matches
    if CHECKPOINT.exists():
        try:
            model.load(str(CHECKPOINT))
            print(f"Warm-starting from {CHECKPOINT}")
            # Re-apply Muskingum init (checkpoint may have stale K values)
            with torch.no_grad():
                model.log_K_musk.copy_(_raw)
            print(f"  (re-initialized Muskingum K from reach lengths)")
        except Exception as e:
            print(f"Cannot warm-start ({e}), starting fresh")
