"""Efficient chunked training with NERF and proper initialization."""
import argparse
import logging
import os
import tomllib
...
def main():
    parser = argparse.ArgumentParser(description="Chunked training")
    ...
    CHECKPOINT = Path("notebooks/slso/checkpoints/chunked.pt")

    # Shorter training period for faster iteration
    ...
    # Load forcing for shorter period
    forcing = extract_forcing(
        ...
        date_end=DATE_END,
        cache_nc=Path("/tmp/forcing_chunked.nc"),
        device=device,
    )

    ds_time = xr.open_dataset(Path("/tmp/forcing_chunked.nc"))
    ...
    # CRITICAL: Use NERF with proper initialization
    model = YHydro(
        n_nodes=n_nodes,
    ...
    print("\n=== Chunked Training ===")
    print(f"Training: {DATE_START} to {TRAIN_END}")
    print(f"Validation: {VAL_START} to {DATE_END}")

    CHUNK_SIZE = 90  # 3 months at a time
    WARMUP = 30  # 1 month warmup
    ...
    for epoch in range(100):
        ...
        for chunk_i in range(n_chunks):
            ...
            chunk_withdrawals = withdrawals  # Pass full withdrawals object
            ...
            Q_sim, state = model.simulate(
                ...
                withdrawals=chunk_withdrawals,
                day_of_year=chunk_doy,
            )

            # Only use non-warmup period for loss
