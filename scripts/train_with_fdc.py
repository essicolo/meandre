"""Training with FDC loss and proper warm initialization."""
import argparse
import logging
import os
import tomllib
...
def main():
    parser = argparse.ArgumentParser(description="Train with FDC loss")
    ...
    CHECKPOINT = Path("notebooks/slso/checkpoints/fdc.pt")
    ...
    print(f"Training with FDC loss and warm initialization")
    ...
    # Model - static mode for simplicity
    model = YHydro(
        n_nodes=n_nodes,
        ...
        dropout=0.0,
        param_mode="static",  # 28 parameters like Hydrotel
    ).to(device)
